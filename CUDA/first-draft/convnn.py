...
output = convnn_cuda.prime_and_conv(v, attn_matrix, K)
...
class PrimeConvFunction(torch.autograd.Function):
    ...
    def forward(ctx, v, attn, conv_w, K):
        output = convnn_cuda.prime_conv_forward(v, attn, conv_w, K)
        ctx.save_for_backward(v, attn, conv_w)
        ctx.K = K 
        return output 

    @staticmethod
    def backward(ctx, grad_output):
        v, attn, conv_w = ctx.saved_tensors
        grad_v, grad_attn, grad_w = convnn_cuda.prime_conv_backward(
            grad_output, v, attn, conv_w, ctx.K
        )
        return grad_v, grad_attn, grad_w, None
